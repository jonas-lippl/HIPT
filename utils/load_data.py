...
def load_lymphoma_data(batch_size, mode='train', ppb=10000):
    path_to_data = f"/sandbox_data/10_blobs_{ppb}_ppb"
    ...
    transform = transforms.Resize((224, 224), antialias=None)
    ...
    data_iter = IterableBlobDataset(path_to_data, blobs_paths, filename_splits, total_train_len, total_val_len,
                                    total_test_len, mode=mode, transform=transform)
    return DataLoader(data_iter, batch_size=batch_size, drop_last=False, num_workers=1, prefetch_factor=8)
