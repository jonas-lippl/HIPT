...
def train_loop(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None, gc=32):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if hasattr(model, "num_clusters"):
            ...
            data, cluster_id, label = data.to(device, non_blocking=True), cluster_id, label.to(device,
                                                                                               non_blocking=True)
        else:
            data, label = batch
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            if hasattr(model, "num_clusters"):
                ...
                data, label = data.to(device), label.to(device)
                cluster_id = None
            logits, Y_prob, Y_hat, _, instance_dict = model(h=data, cluster_id=cluster_id, label=label,
                                                            instance_eval=True)
            acc_logger.log(Y_hat, label)
