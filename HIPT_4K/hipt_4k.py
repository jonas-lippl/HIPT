...
class HIPT_4K(torch.nn.Module):
    ...
    def _get_region_attention_scores(self, region, scale=1):
        ...
        attention_256 = nn.functional.interpolate(attention_256, scale_factor=int(16 / scale),
        ...
        attention_4k = \
            nn.functional.interpolate(attention_4k.unsqueeze(0), scale_factor=int(256 / scale), mode="nearest")[
                0].cpu().numpy()
    ...
    def get_region_attention_heatmaps(self, x, offset=128, scale=4, alpha=0.5,
                                      cmap=cmap_map(lambda x: x / 2 + 0.5, plt.cm.jet), threshold=None):
        r"""
        ...
        if threshold != None:
            for i in range(6):
                ...
                region256_hm = cv2.addWeighted(color_block256, alpha, save_region.copy(), 1 - alpha, 0,
                                               save_region.copy())
        ...
        if False:
            for j in range(6):
                ...
                region4k_hm = cv2.addWeighted(color_block4k, alpha, save_region.copy(), 1 - alpha, 0,
                                              save_region.copy())
        ...
        for j in range(6):
            ...
            for i in range(6):
                ...
                score = (score4k * overlay4k + score256 * overlay256) / (
                        overlay4k + overlay256)  # factorize(score256*score4k)
                color_block = (cmap(score) * 255)[:, :, :3].astype(np.uint8)
                region4k_256_hm = cv2.addWeighted(color_block, alpha, save_region.copy(), 1 - alpha, 0,
                                                  save_region.copy())
